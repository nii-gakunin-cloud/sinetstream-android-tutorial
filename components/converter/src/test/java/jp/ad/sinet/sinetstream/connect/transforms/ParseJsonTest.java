...
public class ParseJsonTest {
    ...
    @Test
    public void testByteBufferWithPositionAndLimit() {
        xformValue.configure(Collections.emptyMap());
        byte[] padding = new byte[] { 0, 0, 0 };
        byte[] backing = new byte[padding.length + JSON_BYTES.length + padding.length];
        System.arraycopy(padding, 0, backing, 0, padding.length);
        System.arraycopy(JSON_BYTES, 0, backing, padding.length, JSON_BYTES.length);
        ByteBuffer buf = ByteBuffer.wrap(backing, padding.length, JSON_BYTES.length);

        SourceRecord transformed = xformValue.apply(createRecordWithSchema(Schema.BYTES_SCHEMA, buf));
        assertNull(transformed.valueSchema());
        assertInstanceOf(Map.class, transformed.value());
        assertEquals("test-01", ((Map<?, ?>) transformed.value()).get("label"));
        assertEquals(12L, ((Map<?, ?>) transformed.value()).get("value"));
    ...
    public void testDirectByteBuffer() {
        xformValue.configure(Collections.emptyMap());
        ByteBuffer buf = ByteBuffer.allocateDirect(JSON_BYTES.length);
        buf.put(JSON_BYTES);
        buf.flip();
    ...
    public void testReadOnlyByteBuffer() {
        xformValue.configure(Collections.emptyMap());
        ByteBuffer buf = ByteBuffer.wrap(JSON_BYTES).asReadOnlyBuffer();
